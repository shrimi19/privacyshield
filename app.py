...
# Marker appended to redacted PDFs so we can later extract and decrypt payload.
PAYLOAD_MARKER = b"\n%%PRIVACYSHIELD_ENCRYPTED_ORIGINAL_V1%%\n"


def _safe_unlink(path: Path) -> None:
    """Best-effort file cleanup that never breaks a request."""
    try:
        path.unlink(missing_ok=True)
    except Exception:
        # On Windows, files may still be locked by a preview response.
        # Ignore cleanup failures; these are temporary artifacts.
        pass


def _embed_encrypted_original(redacted_pdf_path: Path, original_pdf_path: Path, key: bytes) -> None:
    """Append encrypted original PDF bytes to the redacted PDF file."""
...
def _extract_encrypted_payload(redacted_pdf_bytes: bytes) -> bytes:
    """Extract encrypted payload bytes from a redacted PDF with embedded marker."""
...
def preview(job_id, variant):
    """Serve original or redacted PDF inline for in-browser preview."""
    if not job_id.isalnum():
        return jsonify({"error": "Invalid job id."}), 400
...
def restore_original():
    """Restore original PDF from redacted PDF + user-provided key."""
    if "file" not in request.files:
...
def download_restored(restore_id):
    """Download restored original PDF from a prior restore job."""
...
def preview_restored(restore_id):
    """Preview restored original PDF inline in browser."""
    if not restore_id.isalnum():
        return jsonify({"error": "Invalid restore id."}), 400
